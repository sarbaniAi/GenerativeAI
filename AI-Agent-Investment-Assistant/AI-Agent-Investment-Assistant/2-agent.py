...
# MAGIC - agent: contains the code to build the agent.
# MAGIC - config.yml: contains the configurations.
# MAGIC - driver: logs, evaluate, registers, and deploys the agent.
# MAGIC
...
# MAGIC - Review the contents of config.yml as it defines the tools available to your agent and the LLM endpoint.
# MAGIC
# MAGIC ## Next steps
# MAGIC
...
agent.predict(
    None,
    ChatCompletionRequest(
        messages=[Message(role="user", content="should I invest in tesla stocks?")]
    ),
)
